...
(function() {
	...
	Gridly = (function() {
		...
		Gridly.prototype.compare = function(d, s) {
			...
			if (d.y > s.y + s.h) { return +1; }
			if (s.y > d.y + d.h) { return -1; }
			if ((d.x + (d.w / 2)) > (s.x + (s.w / 2))) { return +1; }
			if ((s.x + (s.w / 2)) > (d.x + (d.w / 2))) { return -1; }
		...
		Gridly.prototype.$sorted = function($elements) {
			...
			return ($elements || this.$('> *')).sort(function(a, b) {
				...
				if ((bPosition !== null) && (aPosition === null)) { return +1; }
				if (!aPosition && !bPosition && $a.index() < $b.index()) { return -1; }
				if (!bPosition && !aPosition && $b.index() < $a.index()) { return +1; }
				if (aPositionInt < bPositionInt) { return -1; }
				if (bPositionInt < aPositionInt) { return +1; }
