/**
 * Gaea
 * based on Ventus © 2012 by Ramón Lamana
...
(function (root, factory) {
	...
	if (typeof define === "function" && define.amd) { // AMD.
		...
		root.Gaea = factory(root.$, root.Handlebars);
	}
	...
	**/
	...
	// Going sloppy to avoid 'use strict' string cost, but strict practices should be followed.
	/* jslint sloppy: true */
	...
	(function(root) {
		...
		**/
		...
		if (typeof module !== "undefined" && module.exports) {
			...
			define("gaea/core/emitter",[],function() { return Emitter; });
		}
	...
	**/
	...
	define("gaea/core/view",["$"], function($, _) {
		...
		var animationEventNames = "animationend oAnimationEnd MSAnimationEnd webkitAnimationEnd";
		var transitionEventNames = "transitionend oTransitionEnd MSTransitionEnd webkitTransitionEnd";
	...
	**/
	...
	define("gaea/tpl/window.tpl",["handlebars"], function(Handlebars) {
	...
	define("gaea/wm/window", ["gaea/core/emitter", "gaea/core/view", "tpl!gaea/tpl/window", "less!gaea/css/window"], function(Emitter, View, WindowTemplate) {
		...
		Window.prototype = {
			...
			**/
			...
			maximize: function() {
				this.el.addClass("maximizing");

				this.el.onTransitionEnd(function() {
					this.el.removeClass("maximizing");
				}, this);
		...
		var DefaultMode = {
			...
			actions: {
				maximize: function(win) {
					win.move(0, 0);
					// win.move(0, 50); offset, for taskbar
				...
				// bad Gapelia hack, should fix at some point
				/*
				...
				*/
				...
				minimize: function(win, restore) {
				// minimize: function(win) {

					var window_id = this.el.find("header h1").html();

					win.el.css("transform", "translate3d(0, 0, 0)");
					win.el.css("display", "none");

					// win.resize(this.el.width(32), this.el.height(32));

					$("#running-apps").on(function () {
						win.resize(this.el.width(32), this.el.height(32));
					}).append('<li id="' + window_id + '" class="minimized-app"><a href="#"></a></li>');

					/*
					$("#running-apps").on(function () {
						win.resize(this.el.width(32), this.el.height(32));
					}).append('<li id="' + window_id + '" class="minimized-app"><a href="#"></a></li>').click(function () {
						wm.createWindow({
							title: "New Wallpaper",
							x: 100,
							y: 100,
					...
					*/
					...
					/*
					$('<li id="' + window_id + '" class="minimized-app"><a href="#"></a></li>').click(function () {
						win.resize(this.el.width(32), this.el.height(32));
					});
					*/

					/*
					$(".minimized-app").click(function () {
						wm.createWindow.fromQuery("#folder-ii", {
							title: "Wallpaper",
							x: 100,
							y: 100,
							width: 550,
							height: 250
						}).open(); // .addClass("current") // .attr("id", "someId");
					});
					*/
					...
					// $(".wm-space").css("z-index", "9999 !important");
					// $(".wm-space").css("z-index", "1111 !important").append('<li id="" class="minimized-app"><a href="#"></a></li>');
					// $("#running-apps").append('<div class="dialog_window_minimized ui-widget ui-state-default ui-corner-all" id="' + dialog_id + '_minimized">' + this.uiDialogTitlebar.find('.ui-dialog-title').text() + '<span class="ui-icon ui-icon-newwin"></div>');

					// $("#running-apps").append('<li id="' + this.$window_id + '" class="minimized-app"><a href="#"></a></li>');
					// win.resize(this.el.width(32), this.el.height(32)).append('<li id="' + this.el.$titlebar + '" class="minimized-app"><a href="#"></a></li>');

					// $("#running-apps").append('<li id="' + window_id + '" class="minimized-app"><a href="#"></a></li>'); // .prop("id", "test");

					/*
					$("#running-apps").each(function () {
						win.resize(this.el.width(32), this.el.height(32));
					}).after('<li id="' + window_id + '" class="minimized-app"><a href="#"></a></li>');
					*/
	...
	**/
	...
	define("gaea/wm/windowmanager",["require","$","gaea/wm/window","gaea/core/view","gaea/wm/modes/default","gaea/wm/modes/expose","gaea/wm/modes/fullscreen"], function(require) {
		...
		var $ = require("$");
		var Window = require("gaea/wm/window");
		var view = require("gaea/core/view");
		var DefaultMode = require("gaea/wm/modes/default");
		var ExposeMode = require("gaea/wm/modes/expose");
		var FullscreenMode = require("gaea/wm/modes/fullscreen");
	...
	**/
	...
	define("gaea",["require","gaea/wm/windowmanager","gaea/wm/window"], function(require) {
